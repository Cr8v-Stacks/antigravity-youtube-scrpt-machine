...
6d. **Enforce Retention Standing Rules for Versus and Best/Top Formats**: (1) Never include standalone 'Contenders' intro blocks or disclaimer sequences that delay the first category; introduce products in rapid 5-second dynamic tags inside the hook or directly inside Category 1; (2) **The 120-Second Payoff Rule**: The first scored category and comparison graphic must appear within 120 seconds of video start; (3) **The Post-Payoff No-Lag Rule**: Never insert an abstract theoretical essay or industry defense immediately after a scorecard reveal; launch instantly into the next category's visceral real-world friction; (4) Move all model/generation caveats (e.g., evaluating Model A when Model B is announced) and methodology disclaimers to the pinned comment and video description.
...
- `references/best_top_format_template.md` for the standing rules, scoring matrix, and reusable template for research-and-owner-feedback 'Best/Top' deep-dive videos.
